...
test('uploaded PDF uses local Tamil notes, mind map and Q&A without cloud requests',async({page})=>{
 ...
 await page.goto('/');await page.getByLabel('Upload PDF file').setInputFiles('tests/fixtures/study.pdf');await expect(page.locator('.document-strip')).toContainText('study.pdf');await page.getByLabel('Preferred language',{exact:true}).selectOption('Tamil');
...
test('local translation can be cancelled and English preview remains usable',async({page})=>{
 await localWorker(page,true);await page.goto('/');await page.getByLabel('Upload PDF file').setInputFiles('tests/fixtures/study.pdf');await expect(page.locator('.document-strip')).toContainText('study.pdf');await page.getByLabel('Preferred language',{exact:true}).selectOption('Tamil');await page.getByRole('button',{name:'Create Short notes',exact:true}).click();await expect(page.getByRole('status')).toContainText('Loading local');await page.getByRole('button',{name:'Cancel translation'}).click();await expect(page.getByRole('alert')).toContainText('cancelled');
