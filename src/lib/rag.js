...
export function chunkPages(pages, size = 150, overlap = 25) {
  ...
  const chunks = [];
  for (const {page, text, title, origin, ocrConfidence} of pages) {
    const words = [...text.matchAll(/\S+/g)];
    for (let start = 0; start < words.length; start += size - overlap) {
      chunks.push({id:`S${chunks.length + 1}`,page,text:text.slice(words[start].index,words[Math.min(start+size,words.length)-1].index+words[Math.min(start+size,words.length)-1][0].length),title:title||detectTitle(text, page),...(origin?{origin}:{}),...(ocrConfidence!=null?{ocrConfidence}:{})});
