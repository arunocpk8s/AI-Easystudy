...
export default function App(){
  ...
  async function upload(file){
    ...
    try{
      const result=await readDocument(file,(page,total)=>setProgress(`Read page ${page} of ${total}`),{ocrEnabled,ocrLanguage,signal:controller.signal,onStatus:setProgress});
  ...
  async function generate(id){
    if(!doc)return;if(!doc.chunks.length){setError('No readable passages are available. Choose the document printed language and re-upload a clearer scan with OCR enabled.');return;}setSelected(id);setView('study');setError('');setSubmitted(false);setRevealed({});setResponses({});
  ...
  return <div className="app-shell reference-layout">
    ...
    <div className="main-shell"><header className="topbar"><span className="breadcrumb">Workspace <Icon name="ChevronRight" size={13}/> <strong>{view==='dashboard'?'Overview':view==='study'?'Study studio':view==='ask'?'Ask your document':view==='process'?'Behind the answer':'Project guide'}</strong></span><div className="top-actions"><span className="local-status"><i/>{mode==='ai'?'AI mode':'Local-first workspace'}</span><button className="icon-button" aria-label="Settings" onClick={()=>setSettings(true)}><Icon name="Settings" size={19}/></button><div className="creator-badge" aria-label="Created by Arun Arumugam, AI Expert"><span className="creator-monogram" aria-hidden="true">AA</span><span className="creator-details"><span className="creator-label">CREATED BY</span><strong>Arun Arumugam</strong><span className="creator-role"><Icon name="Sparkles" size={11}/> AI Expert</span></span></div></div></header>
      <main>
        ...
        <div className="reference-subnav"><button onClick={()=>setView('dashboard')}>Study tools</button><button onClick={()=>setView('ask')}>Ask this document</button><button onClick={()=>setView('docs')}>Visual HLD / LLD</button><span>Class {classLevel} · {subject}</span></div>
        ...
        {view!=='docs'&&<DocumentReadingSummary doc={doc} onPage={page=>{const c=doc.chunks.find(c=>c.page===page);setSource(c||{id:'Page preview',page,sourceLabel:doc.pages.find(p=>p.page===page)?.sourceLabel,title:doc.pages.find(p=>p.page===page)?.sourceLabel||`PDF page ${page}`,text:doc.pages.find(p=>p.page===page)?.text||'No selectable text on this page. Review the original page below.'});}}/>}
        {view==='dashboard'&&<StudentDashboard doc={doc} busy={busy} onUpload={()=>fileRef.current.click()} onDrop={upload} onGenerate={generate} onAsk={()=>setView('ask')} onRemove={()=>resetDocument(null)} classLevel={classLevel} setClassLevel={setClassLevel} subject={subject} setSubject={setSubject} language={language} setLanguage={changeLanguage} ocrEnabled={ocrEnabled} setOcrEnabled={setOcrEnabled} ocrLanguage={ocrLanguage} setOcrLanguage={setOcrLanguage}/>}
