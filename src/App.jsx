...
import {readDocument,UPLOAD_ACCEPT} from './lib/documents.js';
import {embedTexts} from './lib/semantic.js';
import Flashcards from './components/Flashcards.jsx';
import {quizScore} from './lib/quiz.js';
import ImageTextReview from './components/ImageTextReview.jsx';
import {reviewImageDocument} from './lib/image-review.js';
...
export default function App(){
  ...
  const [imageKind,setImageKind]=useState('printed');
  const [ocrEnabled,setOcrEnabled]=useState(true);const [ocrLanguage,setOcrLanguage]=useState('English');const uploadController=useRef(null);
  ...
  useEffect(()=>{fetch('/api/status').then(r=>r.json()).then(setStatus).catch(()=>{});return()=>{uploadController.current?.abort();cancelLocalTranslation();if(urlRef.current)URL.revokeObjectURL(urlRef.current);};},[]);
  useEffect(()=>{
  ...
  function changeLanguage(value){setLanguage(value);setError('');if(value!=='English'&&(!doc?.demo||value==='Hindi')&&mode!=='ai')setMode('local');}
  ...
  function resetDocument(value,nextUrl=''){
    ...
    clearTranslationCache();setDoc(value);setMaterials({});setVectors(null);setAnswer(null);setSource(null);setTrace(null);setRevealed({});setResponses({});setSubmitted(false);
  }
  async function upload(file){
    if(!file)return;uploadController.current?.abort();const controller=new AbortController();uploadController.current=controller;setBusy('upload');setError('');setProgress('Opening document…');
    try{
      const result=await readDocument(file,(page,total)=>setProgress(`Read page ${page} of ${total}`),{ocrEnabled,ocrLanguage,imageKind,signal:controller.signal,onStatus:setProgress});
      resetDocument(result,URL.createObjectURL(file));setView('dashboard');
      setTrace({kind:'Document preparation',stages:[{name:'Extract document text',ms:result.processing.decodeMs,detail:`${result.pages.length} source sections read`},{name:'Browser OCR',ms:result.processing.ocrMs,detail:`${result.ocr.recognized} of ${result.ocr.attempted} image-only pages recognized (${result.ocr.language})`},{name:'Chunk & index',ms:result.chunkingMs,detail:`${result.chunks.length} chunks in session memory`}],evidence:[],mode:'Local document processing'});
    }catch(e){setError(e.message);}finally{setBusy('');setProgress('');if(uploadController.current===controller)uploadController.current=null;if(fileRef.current)fileRef.current.value='';}
  }
  ...
  async function generate(id){
    if(!doc)return;if(doc.needsReview){setError('Review and approve the image transcription before creating study materials.');setView('dashboard');return;}if(!doc.chunks.length){setError('No readable passages are available. Choose the document printed language and re-upload a clearer scan with OCR enabled.');return;}setSelected(id);setView('study');setError('');setSubmitted(false);setRevealed({});setResponses({});
    const effectiveMode=language!=='English'&&(!doc.demo||language==='Hindi')&&mode==='extractive'?'local':mode;
    if(effectiveMode==='local')setMode('local');
    const key=`${id}:${effectiveMode}:${language}:${classLevel}:${subject}`;
    ...
    try{
      const selectionStart=performance.now();const batches=sectionEvidence(doc.chunks);const selectionMs=performance.now()-selectionStart;const generationStart=performance.now();let items,usage={prompt_tokens:0,completion_tokens:0};let generationMs=0;let localTranslationMs=0;
      if(effectiveMode==='ai'){
  ...
  async function ask(event){
    ...
    try{
      ...
      if(effectiveMode==='local'&&/[\u0B80-\u0BFF\u0900-\u097F]/.test(question)){const t=performance.now();searchQuestion=await translateLocalText(question,/[\u0900-\u097F]/.test(question)?'Hindi':'Tamil','English',setProgress,subject);translationMs=performance.now()-t;}
      if(effectiveMode==='ai'&&/[\u0B80-\u0BFF\u0900-\u097F]/.test(question)){
      ...
      if(!evidence.length){items=[{title:'Not enough evidence',body:'No matching passages were found. Try a more specific question or enable multilingual semantic search. I cannot support an answer from this document.',sources:[]}];}
        ...
        items=doc.demo&&language==='Tamil'&&effectiveMode!=='local'?demoMaterials(doc.chunks,'notes','Tamil').filter(item=>evidence.slice(0,2).some(c=>item.sources.includes(c.id))).map(item=>({title:item.title,body:item.definition.text+'\n'+item.keyPoints.slice(0,3).map(p=>p.text).join('\n'),sources:item.sources})):sourceAnswer(evidence,searchQuestion);
      }
      if(effectiveMode==='local'&&language!=='English'){const t=performance.now();items=await translateStudyItems(items,text=>translateLocalText(text,'English',language,setProgress,subject));generationMs=performance.now()-t;}
  ...
  function sources(ids){return <div className="source-links">{ids.map(id=>{const c=doc.chunks.find(x=>x.id===id);return c?<button key={id} onClick={()=>setSource(c)}><Icon name="BookOpen" size={12}/> {c.sourceLabel||`p. ${c.page}`}<span>{id}</span></button>:null;})}</div>;}
  function renderItems(result,isAnswer=false){
    ...
    if(id==='flashcards')return <Flashcards items={result.items} chunks={doc.chunks} language={result.language||language} revealed={revealed} onReveal={i=>setRevealed(p=>({...p,[i]:!p[i]}))} onSource={setSource}/>;
  ...
  return <div className="app-shell reference-layout">
    <aside className="sidebar"><a className="brand" href="#" onClick={e=>{e.preventDefault();setView('dashboard');}}><span className="brand-mark"><Icon name="BookOpen" size={24}/></span>study<span className="brand-light">atlas</span><span className="brand-dot">.</span></a>
      ...
      <nav>{[{id:'dashboard',label:'Overview',icon:'LayoutDashboard'},{id:'study',label:'Study studio',icon:'Sparkles'},{id:'ask',label:'Ask your document',icon:'MessagesSquare'},{id:'process',label:'Behind the answer',icon:'Workflow'},{id:'docs',label:'Project guide',icon:'BookMarked'}].map(item=><button key={item.id} className={view===item.id?'nav-item active':'nav-item'} onClick={()=>setView(item.id)}><Icon name={item.icon}/>{item.label}{item.id==='study'&&<span className="nav-badge">9</span>}</button>)}</nav>
      <div className="sidebar-divider"/><div className="workspace-label">CURRENT DOCUMENT</div>
      {doc?<div className="sidebar-doc"><span className="pdf-icon"><Icon name="FileText" size={18}/></span><div><strong>{doc.name}</strong><small>{doc.pages.length} pages · {doc.demo?'Demo document':'Local session'}</small></div></div>:<p className="sidebar-empty">Upload a study file to begin.</p>}
      <button className="sidebar-upload" onClick={()=>fileRef.current.click()} disabled={!!busy}><Icon name="Plus" size={16}/> Upload a file</button>
      <div className="sidebar-bottom"><div className="privacy-note"><Icon name="ShieldCheck" size={18}/><div><strong>Your document, your space</strong><small>Document processing stays on your device.</small></div></div><button className="profile" onClick={()=>setSettings(true)}><span className="avatar">S</span><span><strong>Student workspace</strong><small>Learning, one topic at a time</small></span><Icon name="Settings" size={17}/></button></div>
    </aside>
    <div className="main-shell"><header className="topbar"><span className="breadcrumb">Workspace <Icon name="ChevronRight" size={13}/> <strong>{view==='dashboard'?'Overview':view==='study'?'Study studio':view==='ask'?'Ask your document':view==='process'?'Behind the answer':'Project guide'}</strong></span><div className="top-actions"><span className="local-status"><i/>{mode==='ai'?'AI mode':'Local-first workspace'}</span><button className="icon-button" aria-label="Settings" onClick={()=>setSettings(true)}><Icon name="Settings" size={19}/></button><div className="creator-badge" aria-label="Created by Arun Arumugam, AI Expert"><span className="creator-monogram" aria-hidden="true">AA</span><span className="creator-details"><span className="creator-label">CREATED BY</span><strong>Arun Arumugam</strong><span className="creator-role"><Icon name="Sparkles" size={11}/> AI Expert</span></span></div></div></header>
      <main>
        ...
        {busy&&<div className="alert progress" role="status"><span className="spinner"/>{progress||'Working…'}<span>Measured timings appear after completion</span>{busy==='upload'?<button className="secondary" onClick={()=>uploadController.current?.abort()}>Cancel document processing</button>:mode==='local'&&<button className="secondary" onClick={()=>cancelLocalTranslation()}>Cancel translation</button>}</div>}
        {mode==='local'&&<div className="alert warning local-translation-info"><Icon name="Download" size={18}/><div><strong>Local English-to-Tamil / Hindi study translation. No API key.</strong> First use downloads a large model (around 900 MB). Translation runs on your device; initial setup may take several minutes. A laptop is recommended. Original source quotes and formulas stay unchanged. These are translated source aids, not new teacher explanations. Machine translation may get technical wording wrong; check the original citations. Physics mode includes a small terminology glossary.</div></div>}
        {doc?.warnings.length>0&&<div className="alert warning" role="alert"><Icon name="AlertTriangle" size={18}/><div><strong>Some content could not be read.</strong> {doc.warnings.slice(0,3).join(' ')} {doc.warnings.length>3?`+ ${doc.warnings.length-3} more`:''} Unrecognized content is excluded from study materials. Review diagrams in the original document.</div></div>}
        {view==='dashboard'&&<ImageTextReview doc={doc} url={url} busy={busy} onConfirm={text=>{const reviewed=reviewImageDocument(doc,text);clearTranslationCache();setDoc(reviewed);setMaterials({});setVectors(null);setAnswer(null);setSource(null);setRevealed({});setResponses({});setSubmitted(false);setError('');setTrace({kind:'Reviewed image transcription',mode:'User-reviewed source text',stages:[{name:'Review and index',ms:reviewed.reviewMs,detail:`${reviewed.chunks.length} passages from the approved transcription`}],evidence:reviewed.chunks});}}/>}
        {view!=='docs'&&view!=='process'&&<DocumentReadingSummary doc={doc} onPage={page=>{const c=doc.chunks.find(c=>c.page===page);setSource(c||{id:'Page preview',page,sourceLabel:doc.pages.find(p=>p.page===page)?.sourceLabel,title:doc.pages.find(p=>p.page===page)?.sourceLabel||`PDF page ${page}`,origin:doc.pages.find(p=>p.page===page)?.origin,text:doc.pages.find(p=>p.page===page)?.text||'No selectable text on this page. Review the original page below.'});}}/>}
        {view==='dashboard'&&<StudentDashboard doc={doc} busy={busy} onUpload={()=>fileRef.current.click()} onDrop={upload} onGenerate={generate} onAsk={()=>setView('ask')} onRemove={()=>resetDocument(null)} classLevel={classLevel} setClassLevel={setClassLevel} subject={subject} setSubject={setSubject} language={language} setLanguage={changeLanguage} ocrEnabled={ocrEnabled} setOcrEnabled={setOcrEnabled} ocrLanguage={ocrLanguage} setOcrLanguage={setOcrLanguage} imageKind={imageKind} setImageKind={setImageKind}/>}
        {view==='study'&&<><div className="studio-controls"><div className="tool-tabs">{FEATURES.map(f=><button key={f.id} className={selected===f.id?'selected':''} onClick={()=>{setSelected(f.id);setSubmitted(false);setRevealed({});setResponses({});}} disabled={!!busy}><Icon name={f.icon} size={15}/>{f.label}</button>)}</div><div className="generation-controls"><label>Output language<select value={language} disabled={!!busy} onChange={e=>changeLanguage(e.target.value)}><option>English</option><option>Tamil</option><option>Hindi</option></select></label><label>Generation mode<select value={mode} disabled={!!busy} onChange={e=>setMode(e.target.value)}><option value="extractive">Source preview / demo</option><option value="local">Local translation — no API key</option><option value="ai">AI explanations</option></select></label><button className="primary" disabled={!!busy||!doc||doc.needsReview} onClick={()=>generate(selected)}><Icon name="Sparkles" size={16}/>{current?'Regenerate / open':'Create material'}</button>{current&&<button className="secondary" onClick={exportMaterial}><Icon name="Download" size={16}/> Export</button>}</div></div>
          <div className="section-heading"><div><h2>{feature.label}</h2><p>{current?`${current.mode==='local'?'Locally translated source study aids — review translation':current.mode==='ai'?'AI-generated — review accuracy':current.mode==='demo'?'Bilingual demo — manually authored':'Source-based document preview — not AI-generated'} · ${current.items.length} items · ${doc.name}`:'Select your settings and create this study material.'}</p></div>{current&&<button className="text-button" onClick={()=>setView('process')}>See process<Icon name="ArrowRight" size={15}/></button>}</div>
        ...
        {view==='ask'&&<><div className="ask-layout"><section className="ask-panel"><span className="tool-icon tone-0"><Icon name="MessagesSquare" size={25}/></span><h2>What would you like to understand?</h2><p>Keep your question specific. Every response starts with evidence from your current document.</p><form onSubmit={ask}><textarea value={question} onChange={e=>setQuestion(e.target.value)} maxLength={1000} placeholder="e.g. How is electric potential different from electric field?" aria-label="Question" required/><div className="ask-settings"><select aria-label="Answer mode" disabled={!!busy} value={mode} onChange={e=>setMode(e.target.value)}><option value="extractive">Source-based answer</option><option value="local">Local translation — no API key</option><option value="ai">AI explanation</option></select><select aria-label="Answer language" disabled={!!busy} value={language} onChange={e=>changeLanguage(e.target.value)}><option>English</option><option>Tamil</option><option>Hindi</option></select><button className="primary" disabled={!!busy||!doc||doc.needsReview}>Find an answer<Icon name="ArrowRight" size={16}/></button></div></form><div className="example-prompts">{['What is electric charge?','Explain Coulomb’s law','Compare field and potential'].map(q=><button key={q} onClick={()=>setQuestion(q)}>{q}<Icon name="ArrowUpRight" size={13}/></button>)}</div></section><aside className="search-panel"><Icon name="Search" size={24}/><h3>Your search engine</h3><p>{vectors?'Hybrid keyword and multilingual semantic retrieval is active.':'Keyword retrieval is active. Enable semantic search for meaning-based and multilingual retrieval.'}</p><button className="secondary" disabled={!!busy||!!vectors||!doc||doc.needsReview} onClick={enableSemantic}>{vectors?'Semantic search enabled':'Enable semantic search'}</button><small>First use downloads an embedding model. Processing runs locally and may take time. Tamil ranking is still limited; review the retrieved sources.</small></aside></div>{answer&&<><div className="section-heading"><h2>{answer.mode==='local'?'Locally translated source answer':answer.mode==='ai'?'Grounded response':answer.mode==='demo'?'Demo / source-based answer':'Source-based answer'}</h2><button className="text-button" onClick={()=>setView('process')}>Inspect retrieval<Icon name="ArrowRight" size={15}/></button></div>{renderItems(answer,true)}</>}</>}
        {view==='process'&&<><RagPipeline doc={doc} vectors={vectors} trace={trace} busy={busy} progress={progress} mode={mode} aiConfigured={status.aiConfigured} studyTool={feature?.label}/>
{trace?<><div className="section-heading"><div><h2>{trace.kind}</h2><p>{trace.mode}{trace.cached?' · Reused cached material; timings are from the original generation':''}</p></div><span className="subtle-badge">Observed measurements</span></div><div className="timeline">{trace.stages.map((s,i)=><div className="timeline-stage" key={i}><span className="stage-check"><Icon name="Check" size={16}/></span><div><h3>{s.name}</h3><p>{s.detail}</p></div><strong>{duration(s.ms)}</strong></div>)}</div>{trace.usage&&<div className="token-stats"><span>Input tokens <strong>{trace.usage.prompt_tokens||0}</strong></span><span>Output tokens <strong>{trace.usage.completion_tokens||0}</strong></span><span>Cost <strong>Not calculated</strong><small>Use current provider pricing</small></span></div>}<div className="section-heading"><h2>Evidence used</h2><span>{trace.evidence.length} passages</span></div><div className="evidence-list">{trace.evidence.map(c=><details key={c.id}><summary><span className="evidence-id">{c.id}</span>{c.title}<span>Page {c.page}{c.score?` · score ${c.score.toFixed(3)}`:''}</span></summary><p>{c.text}</p><button className="text-button" onClick={()=>setSource(c)}>Open source<Icon name="ArrowUpRight" size={14}/></button></details>)}</div></>:<div className="empty-state"><Icon name="Workflow" size={38}/><h3>Your next action will appear here</h3><p>Upload a supported file, create study materials, or ask a question to inspect the measured process.</p></div>}</>}
        {view==='docs'&&<ArchitectureView/>}
        <footer><span className="footer-brand">studyatlas.</span><span>Created by Arun Arumugam · AI Expert</span><span>Always check the source.</span></footer>
      </main>
    </div>
    {settings&&<div className="modal-backdrop" onClick={()=>setSettings(false)}><section className="modal" role="dialog" aria-modal="true" aria-label="Workspace settings" onClick={e=>e.stopPropagation()}><button className="modal-close icon-button" aria-label="Close settings" onClick={()=>setSettings(false)}><Icon name="X"/></button><span className="tool-icon tone-0"><Icon name="Settings"/></span><h2>Workspace settings</h2><label>Study mode<select aria-label="Study mode" value={mode} disabled={!!busy} onChange={e=>setMode(e.target.value)}><option value="extractive">Source preview / demo</option><option value="local">Local translation — no API key</option><option value="ai">AI explanations</option></select></label><p>Local translation downloads its model on first use. Extracted text stays in this browser.</p><p>Cloud AI status: <strong>{status.aiConfigured?'Configured':'Not configured'}</strong></p><label>Workspace access token<input type="password" value={token} onChange={e=>setToken(e.target.value)} autoComplete="off" placeholder="Enter the server workspace token"/></label><p className="settings-note">The token stays in memory. AI mode sends selected extracted text to Groq through the server. The original file remains local. Owner setup: copy .env.example to .env.local, set GROQ_API_KEY and STUDY_ACCESS_TOKEN, then restart the server; on Vercel use project environment variables. Never enter the provider API key here.</p><button className="primary" onClick={()=>{if(status.aiConfigured&&token.trim()&&mode!=='local')setMode('ai');setSettings(false);setError('');}}>Save for this session<Icon name="Check" size={16}/></button><button className="text-button" disabled={!!busy} onClick={()=>{resetDocument(sample());setMode('extractive');setError('');setSettings(false);}}>Load demo notes</button></section></div>}
    {source&&<div className="modal-backdrop" onClick={()=>setSource(null)}><section className="source-modal" role="dialog" aria-modal="true" aria-label="Source evidence" onClick={e=>e.stopPropagation()}><button className="modal-close icon-button" aria-label="Close source" onClick={()=>setSource(null)}><Icon name="X"/></button><div className="eyebrow">SOURCE EVIDENCE · {source.id}</div><h2>{source.title}</h2><p className="source-meta">{doc.name} · {source.sourceLabel||`PDF page ${source.page}`}</p><blockquote>{source.text}</blockquote>{(source.origin==='ocr'||source.origin==='image-reviewed')&&<p className="settings-note">{source.origin==='image-reviewed'?'User-reviewed image transcription — verify corrections against the original image.':'OCR transcription — compare against the original scan, especially equations, symbols and units.'}</p>}{url&&doc.format&&doc.format!=='pdf'?<><a className="text-button" href={url} download={doc.name}>Download original file<Icon name="Download" size={15}/></a>{doc.format==='image'&&<img className="source-image" src={url} alt="Original uploaded image"/>}<p className="settings-note">Source references describe extracted content order, not the original page layout.</p></>:url?<><a className="text-button" href={`${url}#page=${source.page}`} target="_blank" rel="noreferrer">Open original PDF page<Icon name="ArrowUpRight" size={15}/></a><iframe title={`PDF page ${source.page}`} src={`${url}#page=${source.page}`} /></>:<p className="settings-note">This is an original demo passage. Upload a PDF to view original pages.</p>}</section></div>}
  </div>;
}
