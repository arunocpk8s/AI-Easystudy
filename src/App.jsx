...
export default function App(){
  ...
  async function aiRequest(feature,evidence,q){
    const response=await fetch('/api/study',{method:'POST',headers:{'Content-Type':'application/json','x-study-token':token},body:JSON.stringify({feature,language,question:q,evidence:evidence.map(({id,page,title,text})=>({id,page,title,text}))})});
  ...
  async function generate(id){
    ...
    const key=`${id}:${mode}:${language}`;
    if(materials[id]?.key===key){setTrace({...materials[id].trace,cached:true});return;}
    if(mode==='extractive'&&language==='Tamil'){setError('Tamil generation needs AI mode. Extractive mode preserves the original document language.');return;}
    ...
    try{
      ...
      }else{items=extractiveMaterials(doc.chunks,id);generationMs=performance.now()-generationStart;}
      const nextTrace={kind:'Study material generation',mode:mode==='ai'?'AI · section-by-section':'Extractive · source passages',stages:[{name:'Select document evidence',ms:selectionMs,detail:`All ${count} chunks considered; quiz practice is capped in extractive mode`},{name:'Generate material',ms:generationMs,detail:mode==='ai'?`${sectionEvidence(doc.chunks).length} model calls`:'No model calls'},{name:'Total request',ms:performance.now()-started,detail:'Measured wall-clock time'}],evidence:doc.chunks,usage};
      setMaterials(previous=>({...previous,[id]:{items,key,mode,language,trace:nextTrace}}));setTrace(nextTrace);
  ...
  async function ask(event){
    ...
    try{
      ...
      if(vectors){const t=performance.now();[queryVector]=await embedTexts([question],undefined,'query');embeddingMs=performance.now()-t;}
      const retrievalStart=performance.now();const evidence=retrieve(doc.chunks,question,5,vectors,queryVector);const retrievalMs=performance.now()-retrievalStart;
      ...
      if(!evidence.length){items=[{title:'Not enough evidence',body:'No matching passages were found. Try a more specific question or enable multilingual semantic search. I cannot support an answer from this document.',sources:[]}];}
        ...
        if(language==='Tamil')throw new Error('Tamil explanations require AI mode; extractive results use the source language.');
        items=[{title:'Matching source passages',body:'These are retrieved excerpts, not an AI-generated answer. Check whether they address your question.',sources:[]},...evidence.map(c=>({title:c.title,body:c.text,sources:[c.id]}))];
      }
      setAnswer({items,mode});setTrace({kind:'Question answering',mode:vectors?'Hybrid · keyword + multilingual vectors':'Keyword · BM25 baseline',stages:[{name:'Embed question',ms:embeddingMs,detail:vectors?'Multilingual E5':'Skipped in keyword mode'},{name:'Retrieve evidence',ms:retrievalMs,detail:`${evidence.length} matching passages`},{name:'Generate response',ms:generationMs,detail:mode==='ai'&&evidence.length?'1 model call':'No model call'},{name:'Total request',ms:performance.now()-started,detail:'Measured wall-clock time'}],evidence,usage});
    }catch(e){setError(e.message);}finally{setBusy('');setProgress('');}
  }
  ...
  function renderItems(result,isAnswer=false){
    ...
    if(id==='mindmap')return <div className="mindmap"><div className="map-root"><Icon name="Network"/>{doc.name}</div><div className="map-branches">{result.items.map((item,i)=><article key={i} className="map-node"><span className="node-dot"/><h3>{item.title}</h3><p>{item.body}</p>{sources(item.sources)}</article>)}</div></div>;
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="topbar"><span className="breadcrumb">Workspace <Icon name="ChevronRight" size={13}/> <strong>{view==='dashboard'?'Overview':view==='study'?'Study studio':view==='ask'?'Ask your PDF':view==='process'?'Behind the answer':'Project guide'}</strong></span><div className="top-actions"><span className="local-status"><i/>{mode==='ai'?'AI mode':'Local-first workspace'}</span><button className="icon-button" aria-label="Settings" onClick={()=>setSettings(true)}><Icon name="Settings" size={19}/></button><span className="avatar small">S</span></div></header>
      <main>
        <div className="page-heading"><div><div className="eyebrow">MAKE ROOM FOR UNDERSTANDING</div><h1>{view==='dashboard'?'Your next “aha” starts here.':view==='study'?'Less scrolling. More learning.':view==='ask'?'A question is a great place to start.':view==='process'?'See the work behind the answer.':'Built to learn. Documented to explain.'}</h1><p>{view==='dashboard'?'Turn your study material into a clear path forward.':view==='study'?'Source-linked study tools, shaped around your document.':view==='ask'?'Find evidence in your document, then explore the explanation.':view==='process'?'Real stages, real evidence, and measured processing time.':'Architecture, decisions, and a practical operating guide.'}</p></div><button className="primary upload-top" onClick={()=>fileRef.current.click()} disabled={!!busy}><Icon name="Upload" size={17}/> Upload PDF</button></div>
        ...
        {error&&<div className="alert error" role="alert"><Icon name="AlertCircle" size={18}/>{error}<button aria-label="Dismiss error" onClick={()=>setError('')}><Icon name="X" size={16}/></button></div>}
        ...
        {view==='dashboard'&&<>
          <div className="overview-grid"><section className="hero-card"><div className="hero-tag"><Icon name="Sparkles" size={14}/> YOUR PERSONAL STUDY STUDIO</div><h2>A whole chapter.<br/>A little more clarity.</h2><p>From the first read to the final revision,<br className="desktop-break"/> give every topic a place to click.</p><button onClick={()=>{setView('study');}} disabled={!doc}>Explore study tools <Icon name="ArrowUpRight" size={18}/></button><div className="hero-art" aria-hidden="true"><div className="orbit orbit-one"/><div className="orbit orbit-two"/><div className="art-card art-card-back"><Icon name="Network" size={40}/></div><div className="art-card art-card-front"><div className="art-spark"><Icon name="Sparkles" size={22}/></div><div className="art-line long"/><div className="art-line"/><div className="art-line short"/><div className="art-pills"><i/><i/><i/></div></div><div className="floating-star">✦</div></div></section>
          <section className="upload-card" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(!busy)upload(e.dataTransfer.files[0]);}}><div className="upload-circle"><Icon name="FileUp" size={27}/></div><h3>Bring your own material</h3><p>Drop a textbook, chapter, or class notes.<br/>We’ll help you make sense of it.</p><button onClick={()=>fileRef.current.click()} disabled={!!busy}>Choose a PDF <Icon name="Plus" size={16}/></button><small>PDF up to 20 MB · 200 pages · Text-based</small><span className="upload-privacy"><Icon name="LockKeyhole" size={12}/> No PDF uploaded to a server</span></section></div>
          <div className="stats-row">{[{label:'Pages in your document',value:doc?.pages.length||'—',icon:'Files',tone:'green'},{label:'Searchable passages',value:count||'—',icon:'ScanText',tone:'orange'},{label:'Study tools to explore',value:'9',icon:'Sparkles',tone:'purple'},{label:'Source-linked learning',value:'Every tool',icon:'Link',tone:'blue'}].map(s=><div className="stat-card" key={s.label}><span className={`stat-icon ${s.tone}`}><Icon name={s.icon} size={20}/></span><div><strong>{s.value}</strong><small>{s.label}</small></div></div>)}</div>
          <div className="section-heading"><div><h2>Your learning toolkit</h2><p>Choose what helps you understand, remember, or practise.</p></div><span className="subtle-badge">Made from your material</span></div>
          <div className="tool-grid">{FEATURES.map((f,i)=><button className="tool-card" key={f.id} onClick={()=>generate(f.id)} disabled={!!busy||!doc}><span className={`tool-icon tone-${i%5}`}><Icon name={f.icon} size={22}/></span>{i===0&&<span className="recommended">START HERE</span>}<h3>{f.label}</h3><p>{f.description}</p><span className="tool-footer">{materials[f.id]?'Open material':'Explore tool'}<Icon name="ArrowUpRight" size={17}/></span></button>)}</div>
          <div className="document-strip"><span className="pdf-icon"><Icon name="FileText"/></span><div><strong>{doc?.name||'No document yet'}</strong><p>{doc?.demo?'Original demo notes · Try the tools, then upload your own PDF':`${doc?.pages.length||0} pages · Stored only for this browser session`}</p></div>{doc&&<button className="text-button" disabled={!!busy} onClick={()=>{resetDocument(null);}}>Remove <Icon name="Trash2" size={14}/></button>}</div>
          <div className="tip"><Icon name="Lightbulb" size={17}/><span><strong>A small study tip:</strong> Try flashcards after reading your notes. Remembering is different from recognising.</span></div>
        </>}
        {view==='study'&&<><div className="studio-controls"><div className="tool-tabs">{FEATURES.map(f=><button key={f.id} className={selected===f.id?'selected':''} onClick={()=>{setSelected(f.id);setSubmitted(false);setRevealed({});setResponses({});}} disabled={!!busy}><Icon name={f.icon} size={15}/>{f.label}</button>)}</div><div className="generation-controls"><label>Output language<select value={language} disabled={!!busy} onChange={e=>setLanguage(e.target.value)}><option>English</option><option>Tamil</option></select></label><label>Generation mode<select value={mode} disabled={!!busy} onChange={e=>setMode(e.target.value)}><option value="extractive">Extractive · no API</option><option value="ai">AI · Groq</option></select></label><button className="primary" disabled={!!busy||!doc} onClick={()=>generate(selected)}><Icon name="Sparkles" size={16}/>{current?'Regenerate / open':'Create material'}</button>{current&&<button className="secondary" onClick={exportMaterial}><Icon name="Download" size={16}/> Export</button>}</div></div>
          <div className="section-heading"><div><h2>{feature.label}</h2><p>{current?`${current.mode==='ai'?'AI-generated — review accuracy':'Extractive — original source language'} · ${current.items.length} items · ${doc.name}`:'Select your settings and create this study material.'}</p></div>{current&&<button className="text-button" onClick={()=>setView('process')}>See process<Icon name="ArrowRight" size={15}/></button>}</div>
        ...
        {view==='ask'&&<><div className="ask-layout"><section className="ask-panel"><span className="tool-icon tone-0"><Icon name="MessagesSquare" size={25}/></span><h2>What would you like to understand?</h2><p>Keep your question specific. Every response starts with evidence from your current document.</p><form onSubmit={ask}><textarea value={question} onChange={e=>setQuestion(e.target.value)} maxLength={1000} placeholder="e.g. How is electric potential different from electric field?" aria-label="Question" required/><div className="ask-settings"><select aria-label="Answer mode" value={mode} onChange={e=>setMode(e.target.value)}><option value="extractive">Source excerpts</option><option value="ai">AI explanation</option></select><select aria-label="Answer language" value={language} onChange={e=>setLanguage(e.target.value)}><option>English</option><option>Tamil</option></select><button className="primary" disabled={!!busy||!doc}>Find an answer<Icon name="ArrowRight" size={16}/></button></div></form><div className="example-prompts">{['What is electric charge?','Explain Coulomb’s law','Compare field and potential'].map(q=><button key={q} onClick={()=>setQuestion(q)}>{q}<Icon name="ArrowUpRight" size={13}/></button>)}</div></section><aside className="search-panel"><Icon name="Search" size={24}/><h3>Your search engine</h3><p>{vectors?'Hybrid keyword and multilingual semantic retrieval is active.':'Keyword retrieval is active. Enable semantic search for meaning-based and multilingual retrieval.'}</p><button className="secondary" disabled={!!busy||!!vectors||!doc} onClick={enableSemantic}>{vectors?'Semantic search enabled':'Enable semantic search'}</button><small>First use downloads an embedding model. Processing runs locally and may take time. Tamil ranking is still limited; review the retrieved sources.</small></aside></div>{answer&&<><div className="section-heading"><h2>{answer.mode==='ai'?'Grounded response':'Retrieved evidence'}</h2><button className="text-button" onClick={()=>setView('process')}>Inspect retrieval<Icon name="ArrowRight" size={15}/></button></div>{renderItems(answer,true)}</>}</>}
        {view==='process'&&<><div className="process-explainer"><h2>Two paths, one source of truth</h2><div className="flow-row">{['PDF','Extract text','Chunk','Index','Retrieve / cover sections','Generate','Cite'].map((s,i)=><React.Fragment key={s}><span><b>{i+1}</b>{s}</span>{i<6&&<Icon name="ChevronRight" size={15}/>}</React.Fragment>)}</div><p>Questions retrieve a small set of relevant passages. Whole-document study tools cover every chunk in section batches. Citation checks verify source IDs; they do not prove every claim is correct.</p></div>{trace?<><div className="section-heading"><div><h2>{trace.kind}</h2><p>{trace.mode}{trace.cached?' · Reused cached material; timings are from the original generation':''}</p></div><span className="subtle-badge">Observed measurements</span></div><div className="timeline">{trace.stages.map((s,i)=><div className="timeline-stage" key={i}><span className="stage-check"><Icon name="Check" size={16}/></span><div><h3>{s.name}</h3><p>{s.detail}</p></div><strong>{duration(s.ms)}</strong></div>)}</div>{trace.usage&&<div className="token-stats"><span>Input tokens <strong>{trace.usage.prompt_tokens||0}</strong></span><span>Output tokens <strong>{trace.usage.completion_tokens||0}</strong></span><span>Cost <strong>Not calculated</strong><small>Use current provider pricing</small></span></div>}<div className="section-heading"><h2>Evidence used</h2><span>{trace.evidence.length} passages</span></div><div className="evidence-list">{trace.evidence.map(c=><details key={c.id}><summary><span className="evidence-id">{c.id}</span>{c.title}<span>Page {c.page}{c.score?` · score ${c.score.toFixed(3)}`:''}</span></summary><p>{c.text}</p><button className="text-button" onClick={()=>setSource(c)}>Open source<Icon name="ArrowUpRight" size={14}/></button></details>)}</div></>:<div className="empty-state"><Icon name="Workflow" size={38}/><h3>Your next action will appear here</h3><p>Upload a PDF, create study materials, or ask a question to inspect the measured process.</p></div>}</>}
        {view==='docs'&&<div className="guide-grid">{[{title:'High-level design',text:'Local PDF extraction → page-aware chunks → keyword or hybrid search → section-batched generation → source-linked study dashboard.',file:'HLD.md'},{title:'Low-level design',text:'React state owns documents and cache. PDF.js reads pages. BM25 ranks evidence; optional multilingual vectors add RRF fusion. Server validates requests and model JSON.',file:'LLD.md'},{title:'Runbook',text:'Install Node.js, run npm install, then npm run dev. Configure server secrets for AI. Run tests and build before deploying the Vite app and API functions to Vercel.',file:'RUNBOOK.md'},{title:'Quality and boundaries',text:'No OCR or diagram understanding yet. No production authentication or persistent storage. Extractive mode is not an LLM answer. Teacher review remains necessary.',file:'REQUIREMENTS.md'}].map(g=><article className="material-card" key={g.title}><Icon name="BookMarked" size={22}/><h3>{g.title}</h3><p>{g.text}</p><a className="text-button" href={`/docs/${g.file}`} target="_blank" rel="noreferrer">Read full document<Icon name="ArrowUpRight" size={15}/></a></article>)}<article className="material-card"><h3>Tools and integration evidence</h3><p>React, Vite, PDF.js, Transformers.js, Groq, Zod, Lucide, Node test runner, Playwright and Git. GitHub MCP verified Mozilla’s PDF example. Vercel deployment needs a connected account.</p><a className="text-button" href="/docs/TOOLS.md" target="_blank" rel="noreferrer">Tool inventory<Icon name="ArrowUpRight" size={15}/></a></article></div>}
