...
export default function App(){
  ...
  function changeLanguage(value){setLanguage(value);setError('');if(value==='Tamil'&&!doc?.demo&&mode!=='ai')setMode('local');}
  ...
  async function generate(id){
    ...
    const effectiveMode=language==='Tamil'&&!doc.demo&&mode==='extractive'?'local':mode;
    ...
    try{
      ...
      if(effectiveMode==='local'&&language==='Tamil'){const t=performance.now();items=await translateStudyItems(items,text=>translateLocalText(text,'English','Tamil',setProgress,subject));localTranslationMs=performance.now()-t;}
  ...
  async function ask(event){
    ...
    try{
      ...
      if(effectiveMode==='local'&&/[\u0B80-\u0BFF]/.test(question)){const t=performance.now();searchQuestion=await translateLocalText(question,'Tamil','English',setProgress,subject);translationMs=performance.now()-t;}
      ...
      if(effectiveMode==='local'&&language==='Tamil'){const t=performance.now();items=await translateStudyItems(items,text=>translateLocalText(text,'English','Tamil',setProgress,subject));generationMs=performance.now()-t;}
