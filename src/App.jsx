...
export default function App(){
  ...
  async function upload(file){
    if(!file)return;uploadController.current?.abort();const controller=new AbortController();uploadController.current=controller;setBusy('upload');setError('');setProgress('Opening PDF…');
    try{
      const result=await readPdf(file,(page,total)=>setProgress(`Read page ${page} of ${total}`),{ocrEnabled,ocrLanguage,signal:controller.signal,onStatus:setProgress});
      resetDocument(result,URL.createObjectURL(file));setView('dashboard');
      setTrace({kind:'Document preparation',stages:[{name:'Extract PDF text',ms:result.processing.decodeMs,detail:`${result.pages.length} original PDF pages read`},{name:'Browser OCR',ms:result.processing.ocrMs,detail:`${result.ocr.recognized} of ${result.ocr.attempted} image-only pages recognized (${result.ocr.language})`},{name:'Chunk & index',ms:result.chunkingMs,detail:`${result.chunks.length} chunks in session memory`}],evidence:[],mode:'Local PDF processing'});
  ...
  async function generate(id){
    if(!doc)return;if(!doc.chunks.length){setError('No readable passages are available. Choose the PDF printed language and re-upload a clearer scan with OCR enabled.');return;}setSelected(id);setView('study');setError('');setSubmitted(false);setRevealed({});setResponses({});
  ...
  return <div className="app-shell reference-layout">
    ...
    <div className="main-shell"><header className="topbar"><span className="breadcrumb">Workspace <Icon name="ChevronRight" size={13}/> <strong>{view==='dashboard'?'Overview':view==='study'?'Study studio':view==='ask'?'Ask your PDF':view==='process'?'Behind the answer':'Project guide'}</strong></span><div className="top-actions"><span className="local-status"><i/>{mode==='ai'?'AI mode':'Local-first workspace'}</span><button className="icon-button" aria-label="Settings" onClick={()=>setSettings(true)}><Icon name="Settings" size={19}/></button><div className="creator-badge" aria-label="Created by Arun Arumugam, AI Expert"><span className="creator-monogram" aria-hidden="true">AA</span><span className="creator-details"><span className="creator-label">CREATED BY</span><strong>Arun Arumugam</strong><span className="creator-role"><Icon name="Sparkles" size={11}/> AI Expert</span></span></div></div></header>
      <main>
        ...
        <input ref={fileRef} type="file" accept="application/pdf,.pdf" aria-label="Upload PDF file" className="file-input" disabled={!!busy} onChange={e=>upload(e.target.files?.[0])}/>
        ...
        {doc?.warnings.length>0&&<div className="alert warning" role="alert"><Icon name="AlertTriangle" size={18}/><div><strong>Some content could not be read.</strong> {doc.warnings.slice(0,3).join(' ')} {doc.warnings.length>3?`+ ${doc.warnings.length-3} more`:''} Unrecognized content is excluded from study materials. Review diagrams in the original PDF.</div></div>}
        {view!=='docs'&&<DocumentReadingSummary doc={doc} onPage={page=>{const c=doc.chunks.find(c=>c.page===page);setSource(c||{id:'Page preview',page,title:`PDF page ${page}`,text:doc.pages.find(p=>p.page===page)?.text||'No selectable text on this page. Review the original page below.'});}}/>}
