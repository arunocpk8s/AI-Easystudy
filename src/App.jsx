...
export default function App(){
  ...
  function renderItems(result,isAnswer=false){
    ...
    if(id==='mindmap'||id==='roadmap')return <StudyGraph items={result.items} title={doc.name} type={id} chunks={doc.chunks} onSource={setSource}/>;
  ...
  return <div className="app-shell reference-layout">
    ...
    <div className="main-shell"><header className="topbar"><span className="breadcrumb">Workspace <Icon name="ChevronRight" size={13}/> <strong>{view==='dashboard'?'Overview':view==='study'?'Study studio':view==='ask'?'Ask your PDF':view==='process'?'Behind the answer':'Project guide'}</strong></span><div className="top-actions"><span className="local-status"><i/>{mode==='ai'?'AI mode':'Local-first workspace'}</span><button className="icon-button" aria-label="Settings" onClick={()=>setSettings(true)}><Icon name="Settings" size={19}/></button><span className="avatar small">S</span></div></header>
      <main>
        ...
        <div className="reference-subnav"><button onClick={()=>setView('dashboard')}>Study tools</button><button onClick={()=>setView('ask')}>Ask this PDF</button><button onClick={()=>setView('docs')}>Project guide</button><span>Class {classLevel} · {subject}</span></div>
        ...
        {view==='dashboard'&&<StudentDashboard doc={doc} busy={busy} onUpload={()=>fileRef.current.click()} onDrop={upload} onGenerate={generate} onAsk={()=>setView('ask')} onRemove={()=>resetDocument(null)} classLevel={classLevel} setClassLevel={setClassLevel} subject={subject} setSubject={setSubject} language={language} setLanguage={setLanguage}/>}
