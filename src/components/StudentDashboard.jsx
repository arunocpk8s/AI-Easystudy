...
export default function StudentDashboard({doc,busy,onUpload,onDrop,onGenerate,onAsk,onRemove,classLevel,setClassLevel,subject,setSubject,language,setLanguage,ocrEnabled,setOcrEnabled,ocrLanguage,setOcrLanguage}){
  return <section className="student-dashboard" aria-label="Student dashboard">
    <div className="reference-upload" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(!busy)onDrop(e.dataTransfer.files[0]);}}>
      ...
      <p>Upload a text or scanned PDF. Image-only pages can be read using OCR on your device.</p>
      ...
      <div className="ocr-preferences"><label className="ocr-toggle"><input type="checkbox" checked={ocrEnabled} disabled={!!busy} onChange={e=>setOcrEnabled(e.target.checked)}/> Read scanned pages with local OCR</label><label>PDF printed language<select aria-label="PDF printed language" value={ocrLanguage} disabled={!!busy||!ocrEnabled} onChange={e=>setOcrLanguage(e.target.value)}><option>English</option><option>Tamil</option><option>Hindi</option></select></label><small>Choose the language printed in the PDF, independently of your output language. First use downloads OCR files; no API key needed.</small></div>
    ...
    <button className="reference-ask" disabled={!!busy||!doc} onClick={onAsk}><div><h2>Ask this PDF</h2><p>Answers with supporting page citations. Missing keyword evidence produces a “not found” response; semantic candidates still need relevance checks.</p></div><ArrowUpRight size={21}/></button>
