...
export default function RagPipeline({doc,vectors,trace,busy,progress}){
  ...
  const stages=[
    ...
    {title:'Index',description:'Session-memory passages and optional vectors',ms:timings.indexMs,status:doc?(doc.chunks.length?'Ready · browser session index':'No readable passages'):'Waiting for PDF'},
  ...
  return <section className="rag-reference" aria-label="RAG processing pipeline">
    ...
    <div className="pipeline-summary"><Info size={20}/><div><strong>{busy?progress:doc?.demo?'Demo document loaded — upload your own PDF to measure preparation.':doc?(doc.chunks.length?'Your document index is ready for this browser session.':'No readable passages were indexed. Inspect page coverage and OCR warnings.'):'Upload a PDF to start processing.'}</strong><p>Actual time depends on page count, text complexity, model downloads, device speed and API response time. OCR processes printed image-only text locally. Verify transcriptions; table structure, handwriting and diagram interpretation are not supported.</p></div></div>
