...
Follow-up production check: Tamil mindmap passed through the deployed endpoint, while a Hindi roadmap still failed concision checks. Graph character limits were then added directly to the provider schema, prompts request shorter sentences, and provider-side `json_validate_failed` errors are retried once within the same budget. A fresh real-provider Hindi roadmap passed on the first attempt. Regression coverage includes provider-side schema rejection and exact graph limits; final suite has 38 tests.
