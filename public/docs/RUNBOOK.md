...
## Structured AI generation and troubleshooting

The server uses feature-specific strict JSON schemas when the configured model supports them, then validates citations, exact quotes and study-format constraints. It retries validation failure once within a 50-second overall budget. HTTP 504 means a provider timeout, 502 with structure/source wording means both outputs failed validation, and 429 means Groq rate limiting. Wait before retrying a rate-limited request; do not launch several study tools at once. Large documents use multiple section calls and may exceed account quotas. Results are saved only after all batches succeed.

Owner configuration: server-only `GROQ_API_KEY` and `STUDY_ACCESS_TOKEN`; `GROQ_MODEL=openai/gpt-oss-120b` is the tested text-generation model. Keep `.env.local` and `.vercel/workspace-access-token.txt` out of Git. Changing Vercel environment variables requires redeployment. Students enter only the workspace token in Settings.
